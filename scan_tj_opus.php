...
<html lang="zh-CN">
		...
		<div class="col-md-7" style="background-color:rgb(255,255,255);"><!-- 表单循环输出  begin -->
		    <div class="row-fluid" style="padding-top:18px;">
				<div class="span12 text-center fontTitle">
					<span style="font-size:28px;">河</span><h3 style="display:inline;">北北方学院教师著作教材统计表</h3>
				</div>
			</div>
				...
				<form name="form4" method="get">
				    <table class="table table-bordered table-hover">
						...
						<?php
							...
							if($select_date==""){
								$sql = "SELECT * FROM tb_tj_opus WHERE uid = {$_SESSION['uid']} order by id desc LIMIT $startPage,$pageSize";
								...
								$sql = "SELECT * FROM tb_tj_opus WHERE uid = {$_SESSION['uid']} AND date = {$select_date} order by id desc LIMIT $startPage,$pageSize";
								$result = mysqli_query($conn,$sql);
								$row_num = mysqli_num_rows($result);
								/* if($row_num==0){
									$select_date = "";
							...
							for($i=0;$i<$row_num;$i++){
								...
								<tr class="<?php echo $col;?>" >
									<td><input type="radio" name="opus_id"  value="<?php echo $id;?>"/></td>
					                <td><!-- 在标题上添加连接 -->
				                    	<a class="text-link" href="javascript:void(0)" onclick="javascript:window.open('read_opus.php?note_id=<?php echo $id;?>','_blank','toolbar=0, location=0, directories=0, status=0, menubar=0, scrollbars=1, resizable=0, titlebar=1, width=800, height=400, left=288, top=184')"><img src="img/more.png">&nbsp;<?php echo $bookname;?></a>    
					                </td>
					                <td><?php echo $name;?></td>
					                <td><?php echo $date;?></td>
					            </tr>
			...
			<div class="row" style="padding-top:15px;padding-right:50px; padding-bottom:30px;">
				...
				<?php 
					...
					//显示分页条
					//获取数据总条数
					$total_sql = "SELECT COUNT(*) FROM tb_tj_opus WHERE uid = {$_SESSION['uid']}";
					...
					$page_banner .= '<form action="scan_tj_opus.php" method="get" style="display:inline;">';
					...
					$page_banner .= '<button class="btn btn-sm btn-warning" type="submit">确定</button>';
					$page_banner .= '</form>';
					echo $page_banner;
				?>
				</ul>				
	...
	<script>
		function del(){
			if(confirm("此操作将删除选中数据，是否继续？")){
				document.form4.action="do_tj_opus.php";
				}
			}
		function mod(){
			document.form4.action="modify_tj_opus.php";
			}
		function look(){
			document.formyear.action="scan_tj_opus.php";
			}
		function out(){
			document.formyear.action="export_tj_opus.php";
			}
	</script>
