						...
						<table class="table table-hover table-condensed" style="padding: 8px;height:1;padding: 5px;vertical-align: top;border-top: 1px solid #ddd">
							...
							<tbody><!--tbody中为要循环输出的内容  -->
								...
								if($select_author && $select_date <> ""){
									$sql = "SELECT * FROM `tb_tj_theses` WHERE author = '$select_author' AND date = '$select_date' ORDER BY id DESC LIMIT $startPage,$pageSize";
								}elseif($select_date == ""){
									if($select_author == "" ){
										$sql = "SELECT * FROM `tb_tj_theses` ORDER BY id DESC LIMIT $startPage,$pageSize";
									}else{
										$sql = "SELECT * FROM `tb_tj_theses` WHERE author = '$select_author' ORDER BY id DESC LIMIT $startPage,$pageSize";
									}
								}elseif($select_author == ""){
									if($select_date == "" ){
										$sql = "SELECT * FROM `tb_tj_theses` ORDER BY id DESC LIMIT $startPage,$pageSize";
									}else{
										$sql = "SELECT * FROM `tb_tj_theses` WHERE date = '$select_date' ORDER BY id DESC LIMIT $startPage,$pageSize";
									}
								}
								...
								for($i=0;$i<$row_num;$i++){
									...
									<tr class="<?php echo $col;?>" style="padding:0;margin:0;border:0;vertical-align:top;border-top: 1px solid #ddd;vertical-align:bottom;border-bottom: 2px solid #ddd">
						                ...
						                <td style="padding:8px;line-height:0.5;vertical-align:top;border-top:1px solid #ddd"><!-- 在标题上添加连接 -->
					                    	<a href="javascript:void(0)" title="点击查看详情" onclick="javascript:window.open('read_theses.php?note_id=<?php echo $id;?>','_blank','toolbar=0, location=0, directories=0, status=0, menubar=0, scrollbars=1, resizable=0, titlebar=1, width=800, height=420, left=288, top=184')" style="text-decoration:none;"><?php echo $name;?></a>    
						                </td>
						                <td style="padding:8px;line-height:0.5;vertical-align:top;border-top:1px solid #ddd"><?php echo $author;?></td>
					    ...
					    <nav class="pull-right">
						    ...
						    <?php 
								...
								if($select_date == "" && $select_author == ""){
									$total_sql = "SELECT COUNT(*) FROM `tb_tj_theses`";
								}elseif($select_date == "" && $select_author != ""){
									$total_sql = "SELECT COUNT(*) FROM `tb_tj_theses` WHERE author = '$select_author'";
								}elseif($select_date != "" && $select_author == ""){
								    $total_sql = "SELECT COUNT(*) FROM `tb_tj_theses` WHERE date = '$select_date'";
								}elseif($select_date != "" && $select_author != ""){
									$total_sql = "SELECT COUNT(*) FROM `tb_tj_theses` WHERE date = '$select_date' AND author = '$select_author'";
								}
